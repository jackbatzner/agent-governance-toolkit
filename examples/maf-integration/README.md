...
The `.NET` demos now use the real **Microsoft Agent Framework SDK** together with the
shared **AGT Agent Framework adapter** in `agent-governance-dotnet`.

- The scenario projects reference `agent-governance-dotnet/src/AgentGovernance/AgentGovernance.csproj`
- Governed MAF agents are created with `BuildAIAgent(...).WithGovernance(adapter)`
- Policies use the real AGT .NET expression format (`message == ...`, `tool_name == ...`)
...
2. **Capability Sandboxing** — governed MAF tool calls are allowed or blocked by AGT
...
Edit the YAML policy files to change governance behavior. The `.NET` examples use the
real AGT policy expression format:
